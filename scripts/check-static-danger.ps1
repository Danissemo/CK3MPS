...
$Allowlist = @(
    ...
    @{ Path = "source/Updates.cs"; Rule = "File.Copy"; LinePattern = '^File\.Copy\(downloadPath, Path\.Combine\(stagingRoot, "CK3MPS\.exe"\), true\);$'; Reason = "Stages a verified downloaded executable inside the app-owned update workspace." },
    @{ Path = "source/Updates.cs"; Rule = "File.WriteAllText"; Reason = "Writes the app-owned transient self-updater script into the update workspace." },
    @{ Path = "source/Updates.cs"; Rule = "SearchOption.AllDirectories"; Reason = "Searches only the newly extracted update staging directory for CK3MPS.exe." },
    @{ Path = "source/Updates.cs"; Rule = "Directory.Delete"; LinePattern = '^Directory\.Delete\(path, true\);$'; Reason = "Best-effort cleanup of the app-owned temporary update workspace." },
    @{ Path = "source/Updates.cs"; Rule = "Process.Start"; LinePattern = '^Process\.Start\((info|startInfo)\);$'; Reason = "Starts only vetted updater targets: official release page or transient local self-updater script." },
