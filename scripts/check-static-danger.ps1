...
$Allowlist = @(
    ...
    @{ Path = "source/SafeUpdater.cs"; Rule = "File.Move"; LinePattern = '^File\.Move\((source|backup|temporary), (destination|path)\);$'; Reason = "Commits new app files, restores same-volume backups, or atomically commits updater journals." },
