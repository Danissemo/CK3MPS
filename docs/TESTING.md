...
3. Click `Preview` on `Recommended` and verify only intended steps are listed.
4. Run `Check only` and confirm it writes reports without changing user files.
5. Run `Recommended` stabilize and confirm a new restore run appears in `Restore`.
...
9. Re-run `Check only` and confirm reports/readiness reflect the current state.
10. Launch CK3, wait for the main menu, exit, then run `Check only` again and confirm whether launcher/game recreated expected defaults or rewrote stability files.
